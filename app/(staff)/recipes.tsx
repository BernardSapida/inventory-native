...
function checkLocal(
  ...
  for (const ing of recipe.ingredients) {
    const cur = invMap.get(ing.itemId) ?? 0;
    if (cur < ing.quantity * times) missing.push(ing.itemName);
  }
...
export default function StaffRecipes() {
  ...
  function availBadge(recipe: RecipeModel) {
    ...
    const { available } = checkLocal(recipe, invMap, 1);
    return available;
  }
          ...
          <View style={styles.modalSheet}>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              <View style={styles.metaRow}>
                ...
                <Tag label={`Serves ${selected?.servingSize}`} />
                <Tag label={`${selected?.ingredients.length ?? 0} ingredient${(selected?.ingredients.length ?? 0) !== 1 ? 's' : ''}`} />
              </View>
...
function FieldLabel({ children }: { children: React.ReactNode }) {
  const C = useColors();
  return <Text style={{ color: C.textSec, fontSize: 11, fontWeight: '700', letterSpacing: 0.8, marginBottom: 6 }}>{children}</Text>;
}

function Tag({ label }: { label: string }) {
  const C = useColors();
  return (
    <View style={{ backgroundColor: C.surfaceAlt, borderRadius: 20, paddingHorizontal: 12, paddingVertical: 4, borderWidth: 1, borderColor: C.border }}>
