...
export default function AdminProfile() {
  ...
  const menuItems: { icon: keyof typeof Feather.glyphMap; label: string; route: string; desc: string }[] = [
    { icon: 'users', label: 'Staff Management', route: '/(admin)/profile/staff-management', desc: 'Manage staff accounts' },
    { icon: 'archive', label: 'Archived Staff', route: '/(admin)/profile/archived-staff', desc: 'View and restore archived staff' },
    { icon: 'shield', label: 'Roles & Permissions', route: '/(admin)/profile/roles-permissions', desc: 'Configure staff permissions' },
  ];
    ...
    <ScrollView style={styles.root} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.profileCard}>
        <View style={styles.avatar}>
          <Text style={styles.avatarText}>{(user?.fullName?.[0] ?? 'A').toUpperCase()}</Text>
        </View>
      ...
      <View style={styles.infoCard}>
        <InfoRow icon="mail" label="Email" value={user?.email ?? '—'} styles={styles} palette={palette} />
      ...
      <Modal visible={showPwChange} transparent animationType="slide" onRequestClose={() => setShowPwChange(false)}>
        <KeyboardAvoidingView style={styles.modalOverlay} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
          <View style={styles.modalSheet}>
            ...
            <TextInput style={[styles.formInput, { marginBottom: 20 }]} value={confirmPw} onChangeText={setConfirmPw} secureTextEntry placeholder="Re-enter new password" placeholderTextColor={palette.textSec} />
...
function makeStyles(C: ColorPalette) {
  return StyleSheet.create({
    ...
    menuItem: { flexDirection: 'row', alignItems: 'center', backgroundColor: C.surface, borderRadius: 14, padding: 16, marginBottom: 8, borderWidth: 1, borderColor: C.border, gap: 14 },
    menuIconWrap: { width: 38, height: 38, borderRadius: 10, backgroundColor: C.brandSoft, justifyContent: 'center', alignItems: 'center' },
